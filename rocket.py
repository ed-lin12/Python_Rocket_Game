...
class Rocket(object):
    ...
    animation_number = 0
    animation_list = sprites.rocket_list
    counter = 0
    current = sprites.rocket_list[animation_number]
    ...
    def wreck(self):
        ...
        if 5 <= self.crash_counter < 10:
            self.current = sprites.rocket_crash_1
        if 10 <= self.crash_counter < 15:
            self.current = sprites.rocket_crash_2
        if 15 <= self.crash_counter < 20:
            self.current = sprites.rocket_crash_3
        if self.crash_counter >= 20:
            self.current = sprites.rocket_crash_4
            self.crash_counter = 0
            self.wreck_start = False
            self.wrecked = True

    def blink_red(self):
        self.animation_list = sprites.damaged_rocket_list
        self.damaged_counter += 1
        if self.damaged_counter >= 15:
            self.animation_list = sprites.rocket_list
            self.damaged = False
